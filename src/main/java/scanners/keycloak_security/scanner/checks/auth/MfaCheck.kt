...
class MfaCheck : SecurityCheck {
    ...
    override fun run(context: CheckContext): CheckResult {
        ...
        try {
            ...
            // Используем настроенный browser flow из realm (может быть кастомный)
            val realm = context.adminService.getRealm()
            val browserFlowAlias = realm.browserFlow ?: "browser"
            val browserFlow = flows.find { it.alias == browserFlowAlias }
            ...
            if (browserFlow == null) {
                ...
                if (executions.isNullOrEmpty()) {
                    ...
                    if (foundMfa.isEmpty()) {
                        ...
                        // CONDITIONAL — стандартный способ MFA в Keycloak (subflow с "User Configured" condition)
                        // ALTERNATIVE — MFA как один из вариантов
                        val alternativeMfa = foundMfa.filter { execution ->
                            execution.requirement == "ALTERNATIVE"
                        }
                        val hasRequiredOrConditional = foundMfa.any { execution ->
                            execution.requirement == "REQUIRED" || execution.requirement == "CONDITIONAL"
                        }

                        if (!hasRequiredOrConditional && alternativeMfa.isNotEmpty()) {
                            findings.add(
                                Finding(
                                    id = id(),
                                    title = "MFA настроена только как альтернатива",
                                    description = "MFA authenticators установлены как ALTERNATIVE без обязательного или условного требования. " +
                                            "Пользователь может пропустить MFA, выбрав другой способ входа.",
                                    severity = Severity.MEDIUM,
                                    status = CheckStatus.DETECTED,
                                    realm = context.realmName,
                                    evidence = listOf(
                                        Evidence("alternativeAuthenticators", alternativeMfa.joinToString {
                                            "${it.providerId ?: "unknown"}: ${it.requirement ?: "null"}"
                                        })
                                    ),
                                    recommendation = "Используйте CONDITIONAL subflow с условием 'User Configured' или REQUIRED для обязательной MFA"
                                )
                            )
                        }
